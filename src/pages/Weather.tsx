...
export default function Weather() {
  ...
  const handleGetCurrentLocation = () => {
    if (!navigator.geolocation) {
      toast({ variant: "destructive", title: copy.weather.errors.geolocationNotSupported });
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          toast({ variant: "destructive", title: copy.weather.errors.failedCurrentLocation });
        } finally {
          setGeoLoading(false);
          setIsLoading(false);
        }
      ...
      () => {
        ...
        toast({
          variant: "destructive",
          title: copy.weather.errors.locationDenied,
          description: copy.weather.errors.locationDeniedDescription,
        });
      },
    );
  };
  ...
  const handleSearchWithLocation = async (loc: string) => {
    if (!loc.trim()) {
      toast({ variant: "destructive", title: copy.weather.errors.enterLocation });
      return;
    }
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: copy.weather.errors.fetchFailed,
        description: error instanceof Error ? error.message : copy.weather.errors.tryAgain,
      });
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="lg:ml-64">
        <Header title={copy.weather.title} subtitle={copy.weather.subtitle} />
        ...
        <main className="p-4 lg:p-6">
          <div className="flex gap-3 mb-6 lg:mb-8 relative">
            <div className="relative flex-1">
              ...
              <Input
                placeholder={copy.weather.searchPlaceholder}
                className="pl-10"
                value={location}
            ...
            <Button onClick={handleSearch} disabled={isLoading}>
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Globe className="w-4 h-4 mr-2" />{copy.weather.getWeather}</>}
            </Button>
            <Button variant="outline" onClick={handleGetCurrentLocation} disabled={geoLoading}>
              {geoLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Navigation className="w-4 h-4 mr-1" />{copy.weather.myLocation}</>}
            </Button>
          </div>
          ...
          {isLoading && (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-foreground font-medium">{copy.weather.loading.title}</p>
              <p className="text-sm text-muted-foreground">{copy.weather.loading.subtitle}</p>
            </div>
          )}
          ...
          {!isLoading && !weather && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-foreground font-medium mb-2">{copy.weather.empty.title}</p>
              <p className="text-sm text-muted-foreground max-w-md mb-4">{copy.weather.empty.subtitle}</p>

              {recentSearches.length > 0 && (
                <div className="mb-6 w-full max-w-lg">
                  <p className="text-xs font-medium text-muted-foreground mb-2 flex items-center justify-center gap-1">
                    <Calendar className="w-3 h-3" /> {copy.weather.empty.recentSearches}
                  </p>
              ...
              <div className="grid lg:grid-cols-3 gap-4 lg:gap-6 mb-6 lg:mb-8">
                <div className="lg:col-span-1 bg-gradient-to-br from-accent to-accent/80 rounded-xl p-5 lg:p-6 text-accent-foreground">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-sm opacity-80">{copy.weather.currentWeather}</p>
                ...
                <div className="lg:col-span-2 space-y-4">
                  <div className="bg-card rounded-xl border border-border overflow-hidden">
                    <div className="p-3 border-b border-border flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-foreground">{copy.weather.mapTitle}</span>
                  ...
                  <div className="bg-card rounded-xl border border-border p-5">
                    ...
                    <div className="space-y-2">
                      {weather.farmingAlerts.length > 0 ? (
                        ...
                        <p className="text-muted-foreground text-sm">{copy.weather.noAlerts}</p>
                      )}
                    </div>
                  </div>
                </div>
                  ...
                  {weather.forecast.map((day, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <DayIcon className={`w-6 h-6 lg:w-8 lg:h-8 mx-auto mb-2 ${day.condition.includes("Rain") || day.condition.includes("Drizzle") ? "text-accent" : "text-secondary"}`} />
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full ${
                            ...
                            {tip.status === "urgent"
                              ? copy.weather.statusLabels.doNow
                              : tip.status === "pause"
                                ? copy.weather.statusLabels.skip
                                : tip.status === "good"
                                  ? copy.weather.statusLabels.good
                                  : copy.weather.statusLabels.wait}
                          </span>
                        </div>
